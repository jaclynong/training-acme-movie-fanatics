...
property = {
		   ...
		   "mvc.command.name=/", "mvc.command.name=/newsletter/issues"
},
...
public class NewsletterIssuesMVCRenderCommand implements MVCRenderCommand {
	...
	public String render(RenderRequest renderRequest, RenderResponse renderResponse)
		...
		List<NewsletterIssue> newsletterIssues = getNewsLetterIssues(newsletterIssueJournalArticles, newsletterArticles);
		
		Collections.sort(newsletterIssues, new NewsletterIssueComparator(false));
		
		NewsletterIssueContainer.setIssues(newsletterIssues);
		
		Map<String, List<NewsletterIssue>> newsletterIssuePerYearMap = getNewsletterIssuesGroupedByYear(newsletterIssues);
		...
		renderRequest.setAttribute("newsletterIssueYearsCommaDelimitedString", newsletterIssueYearsCommaDelimitedString);
	...
	private List<NewsletterIssue> getNewsLetterIssues(List<JournalArticle> journalArticles, List<NewsletterArticle> newsletterArticles) {
		
		List<NewsletterIssue> newsLetterIssues = new ArrayList<NewsletterIssue>();
	...
	private List<NewsletterArticle> getNewsLetterArticles(List<JournalArticle> journalArticles) {
		...
		List<NewsletterArticle> newsLetterArticles = new ArrayList<NewsletterArticle>();
		//TODO: use interface here instead of concrete implementation
	...
	//TODO: move this method out to a separate class
	private List<NewsletterArticle> getNewsletterArticles(List<NewsletterArticle> newsletterArticles, Integer issueNumber) {
		
		List<NewsletterArticle> newsLetterArticlesForIssueNumber = new ArrayList<NewsletterArticle>();
	...
	//TODO: move this method out to a separate class
	private Map<String, List<NewsletterIssue>> getNewsletterIssuesGroupedByYear(List<NewsletterIssue> newsletterIssues) {
		
		Map<String, List<NewsletterIssue>> newsletterIssuePerYearMap = new HashMap<String, List<NewsletterIssue>>();
